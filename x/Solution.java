...
class Solution {
	public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
		...
		while (!pq.isEmpty()) {
			...
			// 도착지(u)까지의 진행 경로(k_visited) 저장
			// 따로 저장해두진 않지만 비용(price_u)이 가장 저렴한 경로다.
			...
			if (k_visited <= k) {
				...
				if (graph.containsKey(u)) {
					...
					for (Map.Entry<Integer, Integer> v : graph.get(u).entrySet()) {
						// 이미 계산한 경로는 큐에 삽입하지 않는 형태로 풀이 최적화
						// 그러나 아직 계산한 경로가 아니거나, 진행 경로(k_visited)가 기존보다 작다면
						// 도착지(dst)까지는 또 다른 최소 비용을 계산할 가능성이 있으므로 삽입해 한 번 더 계산 진행
